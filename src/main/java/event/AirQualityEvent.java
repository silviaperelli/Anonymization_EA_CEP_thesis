...
public class AirQualityEvent extends BaseRichTuple{
    ...
    public AirQualityEvent(long tupleId, LocalDateTime eventTime, double coLevel, double pt08s1, double nmhc, double c6h6, double pt08s2, double nox, double pt08s3, double no2, double pt08s4, double pt08s5, double t, double rh, double ah, EventType eventType) {
    ...
    // Constructor for the output tuple of the Average Window
    ...
    public static AirQualityEvent eventCreation(String line) {
        try {
            ...
            // Parse tuple ID in the first column
            long id = Long.parseLong(tokens[0].trim());

            // Combine date and time and create a single timestamp
            String date = tokens[1];
            String time = tokens[2].replace('.', ':');
            LocalDateTime timestamp = LocalDateTime.parse(date + " " + time, formatter);

            // Parse all the features
            double coValue = Writer.formatDouble(tokens[3]);
            double pt08s1Val = Writer.formatDouble(tokens[4]);
            double nmhcVal = Writer.formatDouble(tokens[5]);
            double c6h6Val = Writer.formatDouble(tokens[6]);
            double pt08s2Val = Writer.formatDouble(tokens[7]);
            double noxVal = Writer.formatDouble(tokens[8]);
            double pt08s3Val = Writer.formatDouble(tokens[9]);
            double no2Val = Writer.formatDouble(tokens[10]);
            double pt08s4Val = Writer.formatDouble(tokens[11]);
            double pt08s5Val = Writer.formatDouble(tokens[12]);
            double tVal = Writer.formatDouble(tokens[13]);
            double rhVal = Writer.formatDouble(tokens[14]);
            double ahVal = Writer.formatDouble(tokens[15]);

            // EventType setting that work with both the original and modified datasets
            AirQualityEvent.EventType type = EventType.NORMAL;

            // If the event type column exists and has content, parse the string into an Enum constant
            if (tokens.length > 16 && tokens[16] != null && !tokens[16].isEmpty()) {
                try {
                    type = AirQualityEvent.EventType.valueOf(tokens[16].trim().toUpperCase());
                } catch (IllegalArgumentException e) {
                    logger.warn("Unknown EventType '{}', defaulting to NORMAL", tokens[16]);
                    type = AirQualityEvent.EventType.NORMAL;
                }
            }

            return new AirQualityEvent(id, timestamp, coValue, pt08s1Val, nmhcVal, c6h6Val, pt08s2Val, noxVal, pt08s3Val, no2Val, pt08s4Val, pt08s5Val, tVal, rhVal, ahVal, type);
    ...
    public String toString() {
        return String.format(Locale.US,
                "%d;%s;%s;%.6f;%.0f;%.0f;%.1f;%.0f;%.0f;%.0f;%.6f;%.0f;%.0f;%.1f;%.1f;%.4f;%s",
                tupleId,
